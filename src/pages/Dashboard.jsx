...
const Dashboard = () => {
  ...
  return (
    (user === null) ? <div>{nevigate('/login')}</div> :
      ...
      <div style={{backgroundSize: 'cover', backgroundColor:'white'}}>
          ...
          <div style={{ display: 'grid' }}>
            ...
            {!loading && (
              <div style={{display:'flex', justifyContent:'center', justifyItems:'center'}}>
                <button className='btn btn-info' onClick={goToPreviousPage} disabled={currentPage === 1}>
                  Previous Page
                </button>
                ...
                <button className='btn btn-info' onClick={goToNextPage} disabled={currentPage === totalPages}>
                  Next Page
                </button>
              </div>
            )}
...
const Frame = ({ name, price, image, id }) => {
  ...
  const handleCardHover = () => {
    cardStyle.border = '2px solid blue'; // Modify the border style when hovering
  };
  ...
  return (
    <>
      <div key={id} className='product card' onMouseEnter={handleCardHover} onMouseLeave={()=>{}} style={{ height: '15rem', width: '15rem', border: '2px solid black', margin: '20px' }} >
        

      <img onClick={btnClick} src={image} alt="image product" style={{ height: '15rem', width: '15rem', cursor:'pointer', cardStyle }} />
